...
from mylib.PhotoviewFilesServer import PhotoviewFilesServer, Host, Ignore, Folder, File, ScanAction, PutScanAction
...
db_server.set_host( host_fs, "debian-1")

@app.get("/v1/hosts")
def get_hosts() -> List[Host]:
    return db_server.list_hosts( host_id="*", for_api=True)

@app.get("/v1/hosts/{host_id}")
def get_host_item(host_id: int) -> Host:
    return db_server.list_hosts( host_id=host_id, for_api=True)

@app.put("/v1/curr_host/scan")
def put_host_item_scan( scan_action: PutScanAction) -> Optional[int]:
    return db_server.create_action( db_server.curr_host_id, scan_action)
    #return check_for_pending_actions( db_server)

@app.get("/v1/hosts/{host_id}/actions")
def get_host_actions(host_id: int) -> List[ScanAction]:
    return db_server.list_actions( host_id=host_id, for_api=True)

@app.get("/v1/hosts/{host_id}/ignores")
def get_host_ignores(host_id: int) -> List[Ignore]:
    return db_server.list_ignores( host_id=host_id, for_api=True)

@app.get("/v1/hosts/{host_id}/folder")
def get_host_folders(host_id: int) -> List[Folder]:
    return db_server.list_folders( host_id=host_id, for_api=True)

@app.get("/v1/actions")
def get_actions() -> List[ScanAction]:
    return db_server.list_actions( host_id="*", for_api=True)

@app.get("/v1/actions/{action_id}")
def get_actions(action_id: int) -> Union[ None, ScanAction]:
    return db_server.list_actions( action_id, for_api=True)

@app.get("/v1/ignores")
def get_ignores() -> List[Ignore]:
    return db_server.list_ignores( host_id="*", for_api=True)

@app.get("/v1/ignores/{ignore_id}")
def get_ignore_item(ignore_id: int) -> Ignore:
    return db_server.list_ignores( ignore_id=ignore_id, for_api=True)

@app.get("/v1/folders")
def get_folderes() -> List[Folder]:
    return db_server.list_folders( host_id="*", for_api=True)

@app.get("/v1/folders/{folder_id}")
def get_foldere_item(folder_id: int) -> Folder:
    return db_server.list_folders( folder_id=folder_id, for_api=True)

@app.get("/v1/folders/{folder_id}/files")
def get_folder_files() -> List[File]:
    return db_server.list_files( folder_id=folder_id, for_api=True)

@app.get("/v1/files")
def get_files() -> List[File]:
    return db_server.list_files( host_id="*", for_api=True)

@app.get("/v1/files/{file_id}")
def get_files_item(file_id: int) -> File:
    return db_server.list_files( file_id=file_id, for_api=True)


def check_for_removed_items( db, container_subtree_to_check):
    #print( f"{container_subtree_to_check = }")
...
def check_for_pending_actions() -> None:
    print("CHECK START")
    start = datetime.now()
    scan_actions = get_host_actions( db_server.curr_host_id)
    print( f"{len(scan_actions) = } {db_server.curr_host_id = }")
    if scan_actions is None or len(scan_actions) == 0:
        print("CHECK END")
        return None
    #print( f"{scan_actions = }")
    scan_action = scan_actions[0]
    subtree = None
    if scan_action['subtree']:
        subtree = scan_action['subtree']
    if scan_action['for_removed']:
        check_for_removed_items( db_server, subtree)
    if scan_action['for_new_or_updated']:
        check_for_removed_items( db_server, subtree)
        check_for_new_or_updated_items( db_server, subtree, scan_action['add_missing_sha'])
    last_id = db_server.delete_action( scan_action['action_id'])
    b = datetime.now()
    print("CHECK END", b-start)
